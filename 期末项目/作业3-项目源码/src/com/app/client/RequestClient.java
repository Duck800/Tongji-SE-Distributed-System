...
public class RequestClient {
                    ...
                    for (String res : result.split(",")) {
                        ...
                        while (t >= 1) {
                            ...
                            if (m.find()) {
                                String st = m.group(1);
                                if (t == 2)
                                    chunkIdx = Integer.valueOf(st);
                                else
                                    queryRes = Integer.valueOf(st);
                            }
    ...
    /**
     * 实现与服务端交互的查询操作的主要功能函数
     *
     * @param type 查询类型：common（默认方式）或 hash（哈希表方式）
     * @return 查询结果
    ...
    public int[] search(String type) throws InterruptedException {
        ...
        for (int i = 0; i < serverInfo.serverNum; i++) {
            RunnableSearch r = new RunnableSearch("t" + i, type, serverInfo.ipList[i], serverInfo.queryPort,
                    threadSignal);
            r.start();
        }
